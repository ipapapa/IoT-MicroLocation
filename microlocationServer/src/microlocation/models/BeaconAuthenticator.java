...
public class BeaconAuthenticator 
{
	...
	//to authenticate beacons from database and then return
	{
		...
		String currentIBeacon = uuid + "_" + major + "_" + minor; // iBeacons stored in database in this manner
		
    	for(int i = 0; i < currentRoomsIbeacons.currentRoomsIBeaconsArray.length; i++)
    		{
    			if(currentRoomsIbeacons.currentRoomsIBeaconsArray[i].equals(currentIBeacon)){
    				success = true;
    				break;
    			}
    	...
    	if(success == false)
    		{
    		cloudant cloudant_connect = new cloudant();
    		try
    			{
    				cloudant_connect.getNewRoomsIBeacons();
